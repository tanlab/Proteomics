...
for helper in ['GCP','P100','both']:
    for cell_line in ['A549', 'MCF7', 'PC3']:
        for target in ['up', 'dn']:
            ...
            mlflow.set_experiment('autokeras__' + cell_line + '_experiments_min_100_' + helper)
            mlflow.start_run()
            ...
            for idx in range(len(indexes)):
                ...
                for i, (trn, val) in enumerate(folds):
                    ...
                    model = ak.StructuredDataClassifier(overwrite=True, max_trials=50, loss='binary_crossentropy')
                    model.fit(trn_x, trn_y, validation_data=(val_x, val_y),class_weight=weights)

                    clf = model.export_model()
                    clf.save("Autokeras_/" + cell_line + '/' + helper + '/' +'model_fold_' + str(i + 1) + '_' + str(indexes[idx]))
